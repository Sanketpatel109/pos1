...
import { Barcode128, generateBarcode128SvgString } from './Barcode128';
  ...
  return (
    ...
    >
      ...
      <div className="text-center pt-2 space-y-1 text-[10px]">
        <div className="py-1 flex justify-center border-y border-dashed border-black">
          <Barcode128
            value={order.orderNumberFormatted ? `ORD-${order.orderNumberFormatted}` : `ORD-${order.orderNumber}`}
            width={1.3}
            height={32}
            displayValue={true}
            fontSize={10}
...
export function generateThermalReceiptHtml(order: Order, shopSettings: ShopSettings): string {
  ...
  return `
    ...
    <div class="text-center" style="font-size: 9px; padding-top: 4px;">
      <div style="border-top: 1px dashed #000; border-bottom: 1px dashed #000; padding: 4px 0; display: flex; justify-content: center; align-items: center;">
        ${generateBarcode128SvgString(
          order.orderNumberFormatted ? `ORD-${order.orderNumberFormatted}` : `ORD-${order.orderNumber}`,
