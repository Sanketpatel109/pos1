...
export const TrainingVideosModal: React.FC<TrainingVideosModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 bg-black/40 backdrop-blur-xs animate-in fade-in duration-150">
      <div className="bg-white rounded-2xl w-full max-w-md border border-border shadow-2xl overflow-hidden flex flex-col max-h-[90vh] text-foreground">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-2.5 no-scrollbar">
          {guides.map((g, idx) => (
            <div
              key={idx}
              className="bg-white border border-border rounded-xl p-3 flex items-center justify-between gap-3 shadow-2xs hover:border-foreground transition-all"
            >
              <div className="w-9 h-9 rounded-lg bg-secondary text-foreground flex items-center justify-center shrink-0">
                <Play className="w-4 h-4 fill-current" />
              </div>
              <div className="min-w-0 flex-1">
                <h3 className="font-bold text-xs text-foreground">{g.title}</h3>
        ...
        <div className="p-3 bg-muted/50 border-t border-border flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-foreground text-background text-white rounded-xl text-xs font-bold hover:bg-black"
          >
            Got It
          </button>
        </div>
