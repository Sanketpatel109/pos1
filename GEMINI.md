...
## MANDATORY: Strict Shadcn UI Component Requirement

Every AI assistant, agent, and engineer working on this repository **MUST STRICTLY** adhere to the following rules for all UI, screen, and component modifications.

### 1. Only Use Pre-Defined Shadcn Components
All visual UI elements (buttons, inputs, cards, dialogs, badges, selects, tables, labels) must **strictly and exclusively** use the Shadcn UI components already installed in `@/components/ui/` (`components/ui/`):

| Component | Import Path | Allowed Variants & Sub-components |
| :--- | :--- | :--- |
| **Button** | `@/components/ui/button` | `default`, `outline`, `secondary`, `ghost`, `destructive`, `link`; sizes: `default`, `sm`, `lg`, `icon`, `icon-lg` |
...
- **NO Inlined Non-Standard Styles**: Do not override canonical Shadcn layouts or inject arbitrary borders/shadows that distort official Shadcn components (e.g., maintain standard `Card`, `Button`, and `Dialog` conventions).
- **NO New UI Components Without Direct User Approval**: If a new primitive is needed from Shadcn (e.g. `Tabs`, `DropdownMenu`, `Popover`), it must be installed using standard Shadcn tooling into `components/ui/` and adhere to the design system.
